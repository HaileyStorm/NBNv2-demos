...
For Multiplication, the task profile still leaves PPO disabled on task selection, but pre-fills the PPO controls with the best current swept defaults from `artifacts/ppo-sweeps/20260611T021142Z`: combined mode, population `64`, `24` rollout ticks, `1` rollout batch, `2` epochs, `2` minibatch size, and `basics.record_score` reward feedback. Enabling artifact PPO for Multiplication also selects direct brain reward-control so the UI uses the recommended combined mode without starting PPO automatically. That reward favors ready balanced/tolerance accuracy over raw fitness. The PPO sweep script uses the same richer Multiplication mutation band as the UI profile, including function mutation and the larger axon/parameter/strength deltas.
