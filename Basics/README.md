...
- Multiplication now keeps raw overall tolerance accuracy in the primary `task_accuracy`/`tolerance_accuracy` fields, but also reports `edge_tolerance_accuracy`, `interior_tolerance_accuracy`, and `balanced_tolerance_accuracy`. Multiplication fitness uses an interior-biased balanced view plus a weak-side harmony term so `min(a,b)`-style edge memorization does not masquerade as real multiplication progress while near-flat interior progress can still benefit from edge recovery.
...
For Multiplication, the task profile still leaves PPO disabled on task selection, but pre-fills the PPO controls with the best current swept defaults from `artifacts/ppo-sweeps/20260611T021142Z`: combined mode, population `64`, reproduction run count `2`, `24` rollout ticks, `1` rollout batch, `2` epochs, `2` minibatch size, `50` maximum generations, and `basics.record_score` reward feedback. Enabling artifact PPO for Multiplication also selects direct brain reward-control so the UI uses the recommended combined mode without starting PPO automatically. That reward favors ready balanced/tolerance accuracy over raw fitness. The UI and sweep share the same Multiplication mutation band, unconstrained initial seed shape, evented output readiness policy, and Medium/harness scheduling baseline used by the winning sweep row.
...
python3 tools/sweep_multiplication_ppo.py --search optuna --optuna-trials 10 --seed-summary artifacts/ppo-sweeps/20260611T021142Z/summary.json --narrow-from-seed --ppo-modes artifact,combined --rollout-ticks 16,20,24,28,32 --rollout-batches 1,2 --epochs 2,3,4 --minibatch-sizes 2 --population 64 --max-concurrent-brains 64 --max-generations 50 --trial-timeout-seconds 900 --optuna-timeout-seconds 3600 --optuna-prune-min-generation 12
```

The sweep script automatically raises `ReproductionRunCount` to the largest requested rollout batch count so batch settings are not silently capped by the generation scheduler. It retries zero-generation IO connection or infrastructure failures with a fresh client endpoint, records completed-generation slopes for tie-breaking, and aborts only after repeated consecutive runtime liveness/capacity failures. Use `--shuffle-seed` and `--repeat-count` when comparing grid settings that may be confounded by runtime state or row ordering. For Optuna, `--seed-summary` enqueues high-ranking previous rows, `--narrow-from-seed` keeps top seed values plus adjacent values from the requested lists, and generation progress is reported to Optuna so weak sampled trials can be gracefully pruned after `--optuna-prune-min-generation`. Direct-only mode runs once per population because artifact PPO rollout, batch, epoch, and minibatch settings are inactive there. Live harness trials also clean up retained final best-candidate brains after report capture so repeated sweeps do not accumulate export-only brains in the runtime.

Basics UI shares the in-session evaluation safety nets: per-brain spawn/output retries, output readiness diagnostics, and startup vector-liveness detection. When every evaluation in a generation fails because startup output vectors never arrive (`vector_missing` with zero vectors seen), the UI stops the run as a runtime liveness issue and retains the best-so-far candidate when available instead of reporting a generic task failure. The sweep script still owns outer process-level behavior such as fresh harness attempts, subprocess/client restarts, and consecutive-infrastructure-failure aborts.
